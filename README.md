...
| `maxRecallChars`   | `number`  | `16000`             | Unified databases: upper bound on the recalled context per turn (and on the `hydradb_query` result). Long result bodies are shortened first, each marked with its id, so headings, ids and citation labels are kept; only if the answer's structure alone is over the bound is its tail cut, with a note. `0` = no bound |
