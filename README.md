...
On a unified database (PRO-1618) the body inside the tags is instead the server-built `llm_prompt` from `POST /query`, injected verbatim: a `=== CONTEXT ===` section with citation labels (`[1]`, `[R1]`, `[P1]`), then `=== FORCEFUL RELATIONS ===` (context the author linked to a result at ingest time, not matched by relevance) and `=== GRAPH ===` when there is anything to show. The `hydradb_query` tool, `/hydradb-query` and `openclaw hydradb query` read the same body's `chunks[].content`, `chunks[].enrichment.text`, `graph[].path_summary` and `forceful_relations[]` for their structured output. Each `graph[]` path carries an `origin`: `query_path` (grown from the query's entities) or `chunk_relation` (the neighbourhood of a returned chunk).
